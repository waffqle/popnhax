...
bool patch_sjis(const char *dllFilename, const char *find, uint8_t find_size, int64_t *offset, char *replace,
                uint8_t replace_size, bool dump_dict) {
...
static void perform_reloc(char *data, int32_t delta, uint32_t ext_base, uint32_t ext_delta) {
    auto headers = (PIMAGE_NT_HEADERS) ((int64_t) data + ((PIMAGE_DOS_HEADER) data)->e_lfanew);
    PIMAGE_DATA_DIRECTORY datadir = &headers->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_BASERELOC];
    auto reloc = (PIMAGE_BASE_RELOCATION) (data + datadir->VirtualAddress);

    while (reloc->VirtualAddress != 0) {
        if (reloc->SizeOfBlock >= sizeof(IMAGE_BASE_RELOCATION)) {
            DWORD relocDescNb = (reloc->SizeOfBlock - sizeof(IMAGE_BASE_RELOCATION)) / sizeof(WORD);
            auto relocDescList = (LPWORD) ((LPBYTE) reloc + sizeof(IMAGE_BASE_RELOCATION));

            for (DWORD i = 0; i < relocDescNb; i++) {
                if (((relocDescList[i]) >> 12) == RELOC_HIGHLOW) {
                    auto *p = (DWORD_PTR *) (data + (reloc->VirtualAddress + ((relocDescList[i]) & 0x0FFF)));
...
#define IPS_READ(_ips_read_dest, _ips_read_size, _ips_read_name) do {\
    ...
    auto headers = (PIMAGE_NT_HEADERS) ((int64_t) data + ((PIMAGE_DOS_HEADER) data)->e_lfanew);
    auto reloc_delta = (DWORD_PTR) ((int64_t) data - headers->OptionalHeader.ImageBase);
    ...
    while (fread(&offset, 1, 3, ips_fp) == 3 && offset != IPS_EOF) {
        ...
        if (trans_base_offset != 0 && offset >= trans_base_offset) {
            ...
            if (trans_rebase == 0) {
                ...
                LPVOID myAlloc = VirtualAllocEx(hProc, nullptr, 16384, MEM_COMMIT, PAGE_EXECUTE_READWRITE);
                if (myAlloc == nullptr) {
...
#define STATE_TRANSLATION 2
    ...
    // Tracking line counts and unfound lines for logging.
    uint16_t line_count = 0;
    std::forward_list<int> unused_lines;
        ...
        switch (state) {
            ...
            case STATE_TRANSLATION:
                if (buffer == ';') {
                    ...
                    if (!patch_sjis(dllFilename, original, orig_size, &curr_offset, translation, arr_idx - 1,
                                    dump_dict)) {
                        printf("Warning: string %s (%s) not found in order, trying again.\n",
                               (char *) original, (char *) translation);

                        // Start over from the beginning if we didn't find a match.
                        curr_offset = 0;

                        if (!patch_sjis(dllFilename, original, orig_size, &curr_offset, translation, arr_idx - 1,
                                        dump_dict)) {
                            LOG("Warning: Skipping line %hu: string '%s' / '%s' not found.\n", line_count, (char *) original, (char *) translation);
                            err_count++;

                            // Track any lines that aren't found
                            unused_lines.push_front(line_count);
                        }
                    }
    ...
    // Print a list of dict lines that weren't found.
    if (!unused_lines.empty()) {
        LOG("Skipped dict lines:\n");
        unused_lines.reverse(); // Nice pretty ascending order.
        for (int &line: unused_lines) {
            LOG("%hi\n", line);
        }
    }
